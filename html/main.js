...
function check_system_offset(now_time){
    system_offset_history.push((now_time + 0.05) - (Date.now() / 1000));
...
function set_position(duration,offset,ref_time){
    ...
    if (Math.abs(position - s) < 0.15){
        if (sound[0]["player"].rate() != 1){
            sound[0]["player"].rate(1);
            console.log("New position: "+position+" Current: "+s+" new rate: 1");
        } else {
            console.log("New position: "+position+" Current: "+s+" same rate: 1");
        }
        return;
